...
export const ConsultationModal: React.FC<ConsultationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 select-text overflow-y-auto print:p-0">
      {/* Frosted Glass Backdrop */}
      <div
        className="fixed inset-0 bg-stone-900/65 backdrop-blur-md transition-opacity animate-fade-in no-print"
        onClick={onClose}
      />

      {/* Main Container */}
      <div
        className={`relative bg-[#faf9f5] border border-[#e6dfd8] rounded-3xl shadow-2xl w-full p-5 sm:p-7 overflow-hidden z-10 my-auto transition-all duration-300 report-print-container ${
          reportActive ? 'max-w-4xl max-h-[92vh] flex flex-col' : 'max-w-2xl'
        }`}
      >
        {/* Decorative Ambient Accents */}
        <div className="absolute -top-16 -right-16 w-48 h-48 bg-[#c2410c]/10 rounded-full blur-3xl pointer-events-none no-print" />
        <div className="absolute -bottom-16 -left-16 w-48 h-48 bg-[#0284c7]/10 rounded-full blur-3xl pointer-events-none no-print" />

        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 sm:top-5 sm:right-5 text-stone-400 hover:text-stone-700 p-2 rounded-2xl hover:bg-[#efe9de] transition-colors cursor-pointer z-20 min-h-[44px] min-w-[44px] flex items-center justify-center no-print"
          title="关闭"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        {reportActive ? (
          <div className="flex flex-col h-full space-y-3 overflow-hidden">
            {/* Report Action Bar */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#e6dfd8] pr-8 no-print">
              <div className="space-y-1">
                <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/25 text-emerald-800 text-[11px] font-mono font-bold">
                  ...
                  <span>{isStreaming ? 'DeepSeek 正在流式精算推演中...' : '推演研报已生成并自动存盘'}</span>
                </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-[#efe9de] hover:bg-[#e4dcce] text-stone-800 text-xs font-semibold flex items-center gap-1.5 border border-[#e6dfd8] transition-colors cursor-pointer shadow-2xs"
                  title="打印或另存为 PDF"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-white hover:bg-stone-100 text-stone-700 text-xs font-semibold flex items-center gap-1.5 border border-[#e6dfd8] transition-colors cursor-pointer shadow-2xs"
                >
                  {copiedReport ? (
                    <>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-white border border-[#e6dfd8] rounded-2xl shadow-inner select-text print:border-none print:shadow-none print:p-0">
              <MarkdownReportRenderer content={streamingText} isStreaming={isStreaming} />
            </div>
